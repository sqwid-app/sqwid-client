...
const Image = styled.div`
	background: linear-gradient(0deg,rgba(132, 32, 28, 0.35), rgba(15, 71, 135, 0.35)), url(${props => props.url});
...
const Card = (props) => {
	...
	return (
		...
		>
			...
			<MetaContainer>
				<h3><label title={props.name}>{props.name}</label></h3>
				<span>
					<p><label title={`Wave ${props.wave}`}>Wave {props.wave}</label></p>
					<p className="price">
						<ReefIcon size={20}/>
			...
			<ItemContainer isEmpty={sacDetails.redeemCards.length===0}>
				{sacDetails.redeemCards.length!==0 ?  (
					<LazyMotion features={domAnimation}>
						{sacDetails.redeemCards.map((card, index) => <Card key={index} {...card} />)}
					</LazyMotion>
				): (
