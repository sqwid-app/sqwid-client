...
const getDistance = (intervalInMinutes) => formatDistance(
	Date.now(),
...
const CardInfo = ({ data, lenderExists }) => {
	const price = (data.loan && data.loan.loanAmount !== '0') && numberSeparator(formatReefPrice(data.loan.loanAmount).toString())
	const fee = (data.loan && data.loan.feeAmount !== '0') && numberSeparator(formatReefPrice(data.loan.feeAmount).toString())
	...
	return (
		<Wrapper>
			...
			{lenderExists ? (
				...
				<BottomWrapper>
					<BottomContainer>
						{fee ? <Fee title={`Interest Amount: ${fee} REEF`}>Returns: <ReefIcon size={`1.125rem`} /><span>{fee}</span></Fee> : null}
						<TimeSpan title={`Time Span: ${timeSpan}`}>{timeSpan}</TimeSpan>
					</BottomContainer>
					<BottomContainer>
						{price ? <Price title={`Loan Amount: ${price} REEF`}><ReefIcon size={`1.5rem`} /><span>{price}</span></Price> : null}
						<Amount title={`Amount: ${data.amount}`}>x{data.amount}</Amount>
					</BottomContainer>
				</BottomWrapper>
			)}
