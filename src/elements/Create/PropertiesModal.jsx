...
const Properties = () => {
	...
	const initialValue = JSON.parse(localStorage.getItem("properties")) || [
		{ key: "", value: "", isFocused: true },
	];
	const [properties, setProperties] = useState(initialValue);
	const addItem = () =>
		setProperties([
			...properties,
			{ key: "", value: "", isFocused: false },
		]);
	useEffect(() => {
		properties.forEach((item, index) => {
			let isAllEmpty = Object.values({
				key: item.key,
				value: item.value,
			}).every(x => x === null || x === "");
	...
	useEffect(() => {
		let isSomeEmpty = properties.some(
			item =>
				Object.values({ key: item.key, value: item.value }).some(
					x => x === null || x === ""
				) && true
		);
		!isSomeEmpty && addItem();
