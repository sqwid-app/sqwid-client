...
const utilityContract = signerOrProvider =>
	new ethers.Contract(
		process.env.REACT_APP_UTILITY_CONTRACT_ADDRESS,
		utilityContractABI,
		signerOrProvider
	);
...
const generateItemsDetails = async (items, collectionId = null) => {
	...
	for (let item of items) {
		...
		try {
			...
			if (collectionId && meta.properties.collection !== collectionId)
				continue;

			let colres = await axios(
				`${process.env.REACT_APP_API_URL}/get/collections/id/${meta.properties.collection}`
			);
			colres = colres.data;
			if (!colres) continue;
...
const fetchMarketplaceItem = async itemId => {
	const res = await axios(
		`${process.env.REACT_APP_API_URL}/get/r/marketplace/fetchMarketItem/${itemId}`
	);
	const { data } = res;
	if (data.error) {
		return null;
