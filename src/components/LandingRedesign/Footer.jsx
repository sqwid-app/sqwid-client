...
const Container = styled.div`
	background: linear-gradient(180deg, rgba(var(--app-theme-value),var(--wave-start-opacity)) , var(--app-theme-primary-dark));
...
const CopyrightSection = () => {
	return (
		<BottomContainer>
			<Copyright>
				&copy; 2021-{new Date().getFullYear()} {constants.APP_WEBSITE.replace("https://", "")} | All Rights Reserved
			</Copyright>
			<IconsContainer />
		</BottomContainer>
	)
...
const FooterContent = () => {
	...
	return (
		<Container>
			...
			<LinksContainer>
				{navLinks.map((link, index) => <NotStyledLink key={index} to={link.link}>{link.name}</NotStyledLink>)}
			</LinksContainer>
			<LinksContainer align="right">
				{technicalLinks.map((link, index) => <NotStyledLink key={index} to={link.link}>{link.name}</NotStyledLink>)}
			</LinksContainer>
			<CopyrightSection />
		</Container>
	);
